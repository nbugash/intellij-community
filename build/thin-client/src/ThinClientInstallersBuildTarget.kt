...
object ThinClientInstallersBuildTarget {
  ...
  private fun buildOptions(): BuildOptions = BuildOptions().apply {
    ...
    // Default to Linux and macOS. BuildOptions defaults to every operating system, and a Windows
    // installer needs NSIS plus a Windows launcher that this fork does not exercise. The standard
    // property still wins, so -Dintellij.build.target.os=all behaves as usual.
  ...
  private fun skippedSteps(): List<String> = listOf(
    ...
    // settings page. A frontend product cannot start on its own yet, because it renders state that
    // a backend supplies. Re-enable this once the client can boot against a host.
    BuildOptions.SEARCHABLE_OPTIONS_INDEX_STEP,
    // A thin client ships no marketplace plugins, so nothing here needs publishing. The step also
    // fails in this checkout for a reason unrelated to this product: patching
    ...
    // 'listBundledPlugins' to enumerate the product's modules, which boots the product on its own.
