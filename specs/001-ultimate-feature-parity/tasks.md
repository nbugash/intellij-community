...
- [ ] T127 [US1] **PARTLY DONE, and the remainder is a real blocker.** A backend product now exists: `BackendProperties`, a build target, a `product-modules.xml`, an application-info and a plugin descriptor, all in fork-owned files plus one more entry in `build/dev-build.json`, which this fork had already changed. The distribution builds and ships this fork's code, 34 backend classes and 57 protocol classes in their own jars. It does not start. Running it found four defects that a green build hid, every one of which also affected the thin client, which had been built and licence-checked and never once launched: no `lib/nio-fs.jar` so the launcher's own `-Xbootclasspath/a` pointed at nothing; the core plugin descriptor module defaulting to `intellij.frontend.split.customization`, a JetBrains Client module Community does not have; no `<icon svg=>` in the application info, which `ApplicationInfo` parsing requires even for a headless host; and `MainImpl` missing because `CommunityModuleSets.essential()` does not carry `intellij.platform.starter`. The first three are fixed in both products. The fourth is open: adding the starter takes the content set from 20 modules to 282 and the build then exits 0 having written no distribution, with no error anywhere. That silent success is the thing to fix first, because it is invisible to any check that reads an exit code
