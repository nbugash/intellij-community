...
- [X] T022 The deferral reason expired when P1.1 landed a transport, so the harness is built. **DEFERRED originally to P1.1, merged with T038.** Create the latency measurement harness at `platform/remoteDev-protocol/testSrc/com/intellij/remoteDev/protocol/LatencyHarness.kt`. No transport exists until P1.1, so a harness built now would measure nothing and would be speculative scaffolding, which constitution Principle 2, Clean Code, forbids under YAGNI. The plan's reason for early measurement was to fix the message shape before it is expensive to change; the shape is now fixed by T016, so the harness must land with T038 and re-open T016 if the measurement demands it
...
- [X] T038 [US1] 1000 samples per row, eight rows, recorded in `latency-baseline.md` with the command that reproduces them. The link is modelled rather than slept, because a test that sleeps 100 ms a thousand times measures the scheduler. `LatencyBaselineTest` asserts nothing about time and prints: a timing assertion on shared hardware fails for reasons unrelated to the code and teaches a team to rerun until green. The harness tests use an injected clock instead and assert relationships that hold on any machine. No warm-up pass, so the first row's 9.13 ms maximum is visible JIT cost that a real first keystroke pays; it does not reach the 95th percentile
...
- [X] T126 [US1] **Not in the original plan, which is the finding.** `HostBootstrap` declares `upload`, `execute` and `forwardPort`, and until now its only implementation in the repository was a test double. Nothing could reach a machine, so an SSH host set up for the end-to-end checks would have had nothing to talk to and the checks would have passed by testing nothing. `SshHostBootstrap` uses the system `ssh` and `scp`, because bootstrap means getting an agent onto a host that has none and every richer transport assumes one is already there: `SshEelDescriptor` in `eel-tcp` speaks to an IJent over TCP, which is the thing that does not exist yet. `StrictHostKeyChecking` defaults to `accept-new` rather than `no`, keeping the refusal of a *changed* key, which is the case that indicates interception. `BatchMode=yes` makes a password-wanting host fail instead of hanging on a prompt nothing will answer. The quoting is the dangerous part: `ssh` joins its arguments and hands the result to the host's shell, so `execute(listOf("echo", "a; rm -rf ~"))` deletes a home directory unless every argument is quoted. Verified by mutation against a real host: with quoting removed, the injected `touch` ran

- [ ] T127 [US1] **PARTLY DONE, one failure left, and one hypothesis disproved.** The backend product builds and ships this fork's code. Seven startup defects fixed, each found by running the product rather than inspecting it, and each also present in the thin client, which had been built and licence-checked and never launched: no `lib/nio-fs.jar`; the core plugin descriptor module defaulting to a JetBrains Client module; no `<icon svg=>`; `MainImpl` belonging in the platform layout rather than the content descriptor; a core plugin id that was not `com.intellij`; the descriptor-carrying module not declared `embeddedModule`; and `ProductMode.BACKEND` being closed to this fork entirely, written up in `docs/fork-platform-changes.md`. **Still failing with "Missing essential plugin: com.intellij".** The hypothesis that a module cannot embed itself was tested and is **wrong**: the descriptor now lives in its own module, `intellij.platform.remoteDev.host.customization`, which embeds the backend module the way `IdeaPlugin.xml` embeds a module other than its own, and the failure is unchanged. Packaging is confirmed correct: `META-INF/RemoteBackendPlugin.xml` is present in `lib/intellij.platform.remoteDev.host.customization.jar`. So this is class loader visibility and neither `loading="embedded"` in the plugin XML nor `embeddedModule()` in the content descriptor puts that jar on the loader that `deprecatedLoadCorePluginForModuleBasedLoader` searches. The next thing to check is whether this fork should be on that code path at all: the build's own product XML generator already emits `<id>com.intellij</id>` as metadata, so the real core descriptor may be the generated product content rather than a hand-written `${platformPrefix}Plugin.xml`, and every Community product takes the other branch of `isProductWithTheOnlyDescriptor`. The separate module is kept because the descriptor and application info do not belong in a code module, but it fixed nothing and should not be read as the fix
...
- [X] T116 **SC-002 is not met, for a structural reason, and its two halves have different answers.** The keystroke budget is 50 ms on a link whose round trip is 100 ms, so half the link costs more than the whole budget before the host has read a byte. That is arithmetic and no optimisation reaches it: the keystroke half is a statement about the architecture, met only by a client that draws the character before the host sees it. This fork has no local echo; `PendingEditLog` replays after an outage, which is FR-015 and a different concern. The completion half already passes by round trip at 100.04 ms against a 300 ms budget, with 200 ms spare, so it needs no speculation and adding some would buy nothing. The protocol is not at stake either way: at a 100 ms link it contributes 2.4 us of 100.0024 ms, or 0.002%. No regression recorded because this is the first baseline
