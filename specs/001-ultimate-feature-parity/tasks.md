...
- [ ] T127 [US1] **PARTLY DONE.** A backend product exists: `BackendProperties`, a build target, a `product-modules.xml`, an application-info and a plugin descriptor, all in fork-owned files plus one entry in `build/dev-build.json`, which this fork had already changed. No fifth upstream file. The distribution builds and ships this fork's code. Running it found five defects a green build hid, each of which also affected the thin client, built and licence-checked and never once launched: no `lib/nio-fs.jar`, because `JetBrainsProductProperties` adds it and FR-002 forbids extending that class, which mixes branding with platform wiring; the core plugin descriptor module defaulting to `intellij.frontend.split.customization`, a JetBrains Client module Community lacks; no `<icon svg=>`, which `ApplicationInfo` parsing requires even for a headless host; `MainImpl` missing, because `essential()` omits `intellij.platform.starter` and that module belongs in the platform layout rather than the content descriptor, having no descriptor XML of its own; and a core plugin id that was not `com.intellij`. All five are fixed. **The open blocker is the next one: the product still reports "Missing essential plugin: com.intellij", so the core plugin descriptor is not being read from `META-INF/RemoteBackendPlugin.xml`.** The file name is non-standard and how the platform is told which descriptor to read has not been established yet. That is the next thing to find out, and it applies to both products
