...
- [ ] T127 [US1] **PARTLY DONE, one failure left.** The backend product builds and ships this fork's code. Seven startup defects fixed, each found by running it rather than inspecting it, and each also present in the thin client, which had been built and licence-checked and never launched: no `lib/nio-fs.jar`; the core plugin descriptor module defaulting to a JetBrains Client module; no `<icon svg=>`; `MainImpl` missing, which belongs in the platform layout and not the content descriptor; a core plugin id that was not `com.intellij`; the descriptor-carrying module not declared `embeddedModule`, so its jar never reached the main class loader; and `ProductMode.BACKEND`, which is closed to this fork entirely and is written up in `docs/fork-platform-changes.md`. **The remaining failure is "Missing essential plugin: com.intellij".** The core descriptor is looked up as `META-INF/{platformPrefix}Plugin.xml` on the main class loader, and that class loader is built from the modules the core plugin header marks `EMBEDDED`. The hypothesis to test next is that a module cannot embed itself: `IdeaPlugin.xml` lives in `community-resources` and embeds a *different* module, `intellij.idea.community.ide.customization`, whereas this descriptor lives in `intellij.platform.remoteDev.backend` and embeds that same module. No product in Community takes this code path, because `isProductWithTheOnlyDescriptor` sends every Community prefix down the `META-INF/plugin.xml` branch instead, so there is no in-repo example to copy
