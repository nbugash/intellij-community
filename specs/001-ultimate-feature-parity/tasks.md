...
- [ ] T089 [P] [US1] Create the refactoring backend module descriptor at `platform/refactoring/backend/intellij.platform.refactoring.backend.iml`
- [ ] T090 [P] [US1] Create the refactoring frontend module descriptor at `platform/refactoring/frontend/intellij.platform.refactoring.frontend.iml`
- [ ] T091 [US1] Declare the refactoring `@Rpc` interfaces in `platform/refactoring/rpc/src/com/intellij/refactoring/rpc/RefactoringApi.kt`
- [ ] T092 [US1] Implement the refactoring backend service in `platform/refactoring/backend/src/com/intellij/refactoring/backend/BackendRefactoringService.kt`
- [ ] T093 [US1] Implement the refactoring frontend through `SplitComponentProvider` in `platform/refactoring/frontend/src/com/intellij/refactoring/frontend/FrontendRefactoringUi.kt`
- [ ] T094 [P] [US1] Create the find frontend module descriptor at `platform/find/frontend/intellij.platform.find.frontend.iml`, since `find.backend` exists alone today
- [ ] T095 [US1] Implement the find and replace frontend in `platform/find/frontend/src/com/intellij/find/frontend/FrontendFindUi.kt`
- [ ] T096 [US1] Register both new frontends in `platform/remoteDev-frontend/resources/META-INF/product-modules.xml`
- [ ] T097 [US1] Regenerate Bazel with `./build/jpsModelToBazelCommunityOnly.cmd` after T089 to T094
- [ ] T098 [US1] Survey every tool window that P1 promises and record its split state in `specs/001-ultimate-feature-parity/split-coverage.md`, since 39 frontend modules against 68 backend modules means gaps remain beyond FR-014
...
- [ ] T103 [US1] Implement session token revocation that ends a session at its next operation in `platform/remoteDev-backend/src/com/intellij/remoteDev/backend/TokenRevocation.kt`
